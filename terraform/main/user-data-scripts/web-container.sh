...
IMAGE_TAG=$(aws ssm get-parameter --name "$SSM_PREFIX/frontend/image_tag" --region "$AWS_REGION" --query "Parameter.Value" --output text)
...
docker run -d \
  ...
  -p 80:3000 \
  -e INTERNAL_API_BASE_URL="$BACKEND_BASE_URL" \
